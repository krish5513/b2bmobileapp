...
public class SyncAgentStockDeliveriesService extends Service {
    ...
    private void syncTripSheetDeliveriesListDataWithServer() {
        try {
            ArrayList<String> unUploadedDeliveriesTripSheetIds = mDBHelper.fetchUnUploadedUniqueAgentStockDeliverys();
            System.out.println("Un Uploaded Agent stock deliveries count Is::: " + unUploadedDeliveriesTripSheetIds.size());
            System.out.println("Un Uploaded Agent stock deliveries count Is 1111::: " + unUploadedDeliveriesTripSheetIds);
            ...
            for (String tripSheetId : unUploadedDeliveriesTripSheetIds) {
                List<TripSheetDeliveriesBean> unUploadedDeliveries = mDBHelper.fetchAllAgentStockDeliveriesList("", tripSheetId);
                ...
                for (int i = 0; i < unUploadedDeliveries.size(); i++) {
                    ...
                    if (i == 0) {
                        ...
                        tripSheetDeliveriesBeanWithProducts.setmTripsheetDelivery_productId(currentDelivery.getmTripsheetDelivery_productId());
                    }
    ...
    private class SyncTripSheetDeliveriesAsyncTask extends AsyncTask<TripSheetDeliveriesBeanWithProducts, Void, Void> {
        ...
        protected Void doInBackground(TripSheetDeliveriesBeanWithProducts... stockList) {
            try {
                ...
                requestObj.put("delivery_date", Utility.formatTime(Long.parseLong(currentDeliveryBean.getmTripsheetDelivery_CreatedOn()), Constants.SEND_DATA_TO_SERVICE_DATE_FORMAT));
                requestObj.put("delivery_no", "");
                ...
                String requestURL = String.format("%s%s%s", Constants.MAIN_URL, Constants.SYNC_TAKE_ORDERS_PORT, Constants.AGENTS_STOCK_DELIVERY_URL);
                ...
                if (responseString != null && !(responseString == "error" || responseString == "failure")) {
                    ...
                    if (resultObj.getInt("result_status") == 1) {
                        ...
                        mDBHelper.updateAgentStockDeliveriesTable(resultObj.getString("insert_no"), currentDeliveryBean.getmTripsheetDelivery_tripId(), currentDeliveryBean.getmTripsheetDelivery_so_id(), currentDeliveryBean.getmTripsheetDelivery_userId(), currentDeliveryBean.getmTripsheetDelivery_productId());
                    }
